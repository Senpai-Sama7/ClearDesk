...
Export respects your current filters — if you're viewing only escalated documents, the export contains only escalated documents.`
  },
  {
    ...
    content: `ClearDesk can sync your documents across devices using Cloudflare KV — no account or login required.

**How it works:**
1. Go to **Settings → Cross-Device Sync**
2. Copy your sync code (a unique ID generated for your browser)
3. On another device, paste the code into the "Enter sync code" field and click **Import**
4. Your documents appear on the new device
          ...
          {content.split('\n\n').map((block, i) => {
            if (block.startsWith('- ')) {
              return <ul key={i} className="space-y-1 ml-1">{block.split('\n').map((line, j) => (
                <li key={j} className="flex gap-2"><span className="text-accent mt-0.5">•</span><span dangerouslySetInnerHTML={{ __html: md(line.replace(/^- /, '')) }} /></li>
              ))}</ul>;
            }
            if (/^\d+\. /.test(block)) {
              return <ol key={i} className="space-y-1 ml-1">{block.split('\n').map((line, j) => (
                <li key={j} className="flex gap-2"><span className="text-accent font-mono text-xs mt-0.5">{j + 1}.</span><span dangerouslySetInnerHTML={{ __html: md(line.replace(/^\d+\.\s*/, '')) }} /></li>
              ))}</ol>;
            }
            return <p key={i} dangerouslySetInnerHTML={{ __html: md(block) }} />;
          })}
        </div>
      )}
    </div>
...
function md(s: string): string {
  return s.replace(/\*\*(.+?)\*\*/g, '<strong class="text-text-primary">$1</strong>');
}
